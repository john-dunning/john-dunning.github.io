...
// this works only if the strings in the array don't contain commas
				// store the array with a marker so we know how to extract it
				// when then cookie is loaded
...
function _Cookie_load()
{
    ...
    for(var i = 0; i < a.length; i++) {
		if (a[i][1].charAt(0) == "[") {
				// this attribute was an array, so remove the [ and turn it back into one
			this[a[i][0]] = unescape(a[i][1].substring(1)).split(",");
		} else {
